...
`flatMap` 可以分成两个方法分别是 `flat` 和 `map` 。

- 参数：当前数组中处理的元素，当前数组的索引，被调用的数组
- 返回值： 一个新的数组，其中每个元素都是回调函数的结果，并且结构深度为 1 。
...
对一个「类似数组」或者是「可迭代对象」创建一个新的、浅拷贝数组实例。

- 参数：伪数组对象或可迭代对象，新数组中的每个元素执行回调函数。
- 返回值：数组实例。

```js
console.log(Array.from('foo')); // 从 string 生成数组
console.log(Array.from([[1,2], [2, 4], [4, 8]])) // 从 Map 生成数组
console.log(Array.from(new Set(['foo', 'bar', 'baz']))); // 从 Set 生成数组
```
